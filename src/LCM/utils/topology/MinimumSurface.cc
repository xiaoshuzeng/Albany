...
std::vector<stk::mesh::Entity> Topology::getClosestNodes(std::vector<std::vector<double> > points)
{
	...
	std::vector<stk::mesh::Entity> entities_D0 = getEntitiesByRank(*(getBulkData()), stk::topology::NODE_RANK);//get all the nodes
...
std::vector<stk::mesh::Entity> Topology::getClosestNodesOnSurface(std::vector<std::vector<double> > points)
{
	...
	//Obtain all the faces of the mesh
        std::vector<stk::mesh::Entity> MeshFaces = getEntitiesByRank(*(getBulkData()), stk::topology::FACE_RANK);
...
std::vector<std::vector<int> > Topology::edgesDirections()
{
		//Get all of the edges
                std::vector<stk::mesh::Entity> setOfEdges = getEntitiesByRank(*(getBulkData()), stk::topology::EDGE_RANK);
...
{
		//Get the faces
                std::vector<stk::mesh::Entity> setOfFaces = getEntitiesByRank(*(getBulkData()), stk::topology::FACE_RANK);

...
//get the segment from the original mesh
  ...
  initial_entities_1D = getEntitiesByRank(*(get_bulk_data()), stk::topology::EDGE_RANK);
  std::vector<stk::mesh::Entity> vector_nodes;

//Adding nodes to the divide segments by half
  std::vector<size_t> request1(get_space_dimension() + 1, 0);
  request1[0] = initial_entities_1D.size();
  addEntities(request1);

  std::vector<stk::mesh::Entity>
  initial_entities_0D = getEntitiesByRank(*(get_bulk_data()), stk::topology::NODE_RANK);
...
//adding segments
  ...
  std::vector<stk::mesh::Entity>
  modified_entities_1D = getEntitiesByRank(*(get_bulk_data()), stk::topology::EDGE_RANK);

  for (unsigned int i = 0; i < initial_entities_1D.size(); i++) {
//Look for all the relations for each segment
    stk::mesh::Entity const* _relations = get_bulk_data()->begin_nodes(initial_entities_1D[i]);
    unsigned const num_relations = get_bulk_data()->num_nodes(initial_entities_1D[i]);
...
//adding the relation between the new segment and the faces
  for (unsigned int i = 0; i < initial_entities_1D.size(); i++) {
    stk::mesh::Entity const* _relations = get_bulk_data()->begin_faces(initial_entities_1D[i]);
    unsigned const num_relations = get_bulk_data()->num_faces(initial_entities_1D[i]);
...
Topology::addCentroid()
{
  ...
  std::vector<stk::mesh::Entity>
  initial_entities_2D = getEntitiesByRank(*(get_bulk_data()), stk::topology::FACE_RANK);

  //Adding nodes to the faces
  std::vector<size_t> request2(get_space_dimension() + 1, 0);
...
Topology::connectCentroid()
{
  ...
  //get the centroid
  std::vector<stk::mesh::Entity>
  modified_entities_0D = getEntitiesByRank(*(get_bulk_data()), stk::topology::NODE_RANK);
...
Topology::addNewFaces()
{
  ...
  for (unsigned i = 0; i < boundary_entities_1D.size(); i++) {
    std::cout << get_bulk_data()->identifier(boundary_entities_1D[i]) << std::endl;
  }
}
void

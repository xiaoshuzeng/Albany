...
Topology::divideSegmentsHalf()
{
  ...
  std::vector<stk::mesh::Entity>
  modified_entities_1D = getEntitiesByRank(
      *(get_bulk_data()),
      stk::topology::EDGE_RANK);

  for (unsigned int i = 0; i < initial_entities_1D.size(); i++) {
    //Look for all the relations for each segment
    stk::mesh::Entity const* _relations = get_bulk_data()->begin_nodes(
        initial_entities_1D[i]);
    unsigned const num_relations = get_bulk_data()->num_nodes(
        initial_entities_1D[i]);

    //add new relation between the new node and the corresponding node
  ...
  for (unsigned int i = 0; i < initial_entities_1D.size(); i++) {
    stk::mesh::Entity const* _relations = get_bulk_data()->begin_faces(
        initial_entities_1D[i]);
    unsigned const num_relations = get_bulk_data()->num_faces(
        initial_entities_1D[i]);
...
Topology::connectCentroid()
{
  ...
  //get the centroid
  std::vector<stk::mesh::Entity>
  modified_entities_0D = getEntitiesByRank(
      *(get_bulk_data()),
      stk::topology::NODE_RANK);
  ...
  for (unsigned int i = 0; i < initial_entities_2D.size(); i++) {
    ...
    //get boundary nodes
    std::vector<stk::mesh::Entity>
    boundary_entities_0D = getBoundaryEntities(
        initial_entities_2D[i],
        stk::topology::NODE_RANK);
...
Topology::addNewFaces()
{
  ...
  //get boundary nodes
  std::vector<stk::mesh::Entity>
  boundary_entities_1D = getBoundaryEntities(
      initial_entities_2D[0],
      stk::topology::EDGE_RANK);

  for (unsigned i = 0; i < boundary_entities_1D.size(); i++) {
    std::cout << get_bulk_data()->identifier(boundary_entities_1D[i])
        << std::endl;
  }
}
void

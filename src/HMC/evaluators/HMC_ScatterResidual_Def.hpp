                              ...
                              const Teuchos::RCP<Albany::Layouts>& dl)
{
  ...
  numDimsBase = dl->node_tensor->dimension(2);
...
ScatterResidual(const Teuchos::ParameterList& p,
  ...
  numDims(ScatterResidualBase<PHAL::AlbanyTraits::Residual,Traits>::numDimsBase)
    ...
    for (std::size_t node = 0; node < this->numNodes; ++node)
      for (std::size_t i = 0; i < numDims; i++)
        for (std::size_t j = 0; j < numDims; j++)
          (*f)[nodeID[node][this->offset + i*numDims + j]] += (this->val)(cell,node,i,j);
  } 
}
...
ScatterResidual(const Teuchos::ParameterList& p,
  ...
  numDims(ScatterResidualBase<PHAL::AlbanyTraits::Jacobian,Traits>::numDimsBase)
{
}
    ...
    for (std::size_t node = 0; node < this->numNodes; ++node) {
      ...
      for (std::size_t i = 0; i < numDims; i++) {
       for (std::size_t j = 0; j < numDims; j++) {
        ...
        row = nodeID[node][this->offset + i*numDims + j];
        if (loadResid) {
...
ScatterResidual(const Teuchos::ParameterList& p,
  ...
  numDims(ScatterResidualBase<PHAL::AlbanyTraits::Tangent,Traits>::numDimsBase)
{
}
    ...
    for (std::size_t node = 0; node < this->numNodes; ++node) {
      for (std::size_t i = 0; i < numDims; i++) {
   	...
   	valptr = &(this->val)(cell,node,i,j);
        ...
        int row = nodeID[node][this->offset + i*numDims + j];
...
ScatterResidual(const Teuchos::ParameterList& p,
  ...
  numDims(ScatterResidualBase<PHAL::AlbanyTraits::SGResidual,Traits>::numDimsBase)
{
}
...
evaluateFields(typename Traits::EvalData workset)
{
  ...
  for (std::size_t cell=0; cell < workset.numCells; ++cell ) {
	...
	for (int block=0; block<nblock; block++)
	  (*f)[block][nodeID[node][this->offset + i*numDims + j]] += valptr->coeff(block);
       }
      }
    }
  }
...
ScatterResidual(const Teuchos::ParameterList& p,
  ...
  numDims(ScatterResidualBase<PHAL::AlbanyTraits::SGJacobian,Traits>::numDimsBase)
{
}
...
ScatterResidual(const Teuchos::ParameterList& p,
  ...
  numDims(ScatterResidualBase<PHAL::AlbanyTraits::SGTangent,Traits>::numDimsBase)
{
}
...
ScatterResidual(const Teuchos::ParameterList& p,
  ...
  numDims(ScatterResidualBase<PHAL::AlbanyTraits::MPResidual,Traits>::numDimsBase)
{
}
...
ScatterResidual(const Teuchos::ParameterList& p,
  ...
  numDims(ScatterResidualBase<PHAL::AlbanyTraits::MPJacobian,Traits>::numDimsBase)
{
}
...
ScatterResidual(const Teuchos::ParameterList& p,
  ...
  numDims(ScatterResidualBase<PHAL::AlbanyTraits::MPTangent,Traits>::numDimsBase)
{
}

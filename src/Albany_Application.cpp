...
void Albany::Application::finalSetUp(const Teuchos::RCP<Teuchos::ParameterList>& params,
  ...
  for(int is=0; is<distParamSIS.size(); is++) {
    ...
    Teuchos::RCP<Epetra_Vector> dist_param(new Epetra_Vector(*node_map));
    ...
    Teuchos::RCP<EpetraDistributedParameter>  parameter(new EpetraDistributedParameter(param_name, dist_param, node_map, overlap_node_map));
...
//      loadBasicWorksetInfo( workset, overlapped_x, overlapped_xdot,
    ...
    for (int ws=0; ws < numWorksets; ws++) {
      ...
      if (nfm!=Teuchos::null)
         nfm->evaluateFields<PHAL::AlbanyTraits::Residual>(workset);
    }
  }
...
//      loadBasicWorksetInfo( workset, overlapped_x, overlapped_xdot,
    ...
    for (int ws=0; ws < numWorksets; ws++) {
      ...
      if (nfm!=Teuchos::null)
        nfm->evaluateFields<PHAL::AlbanyTraits::Jacobian>(workset);
    }
  }
...
//      loadBasicWorksetInfo( workset, overlapped_x, overlapped_xdot,
    ...
    for (int ws=0; ws < numWorksets; ws++) {
      ...
      if (nfm!=Teuchos::null)
        nfm->evaluateFields<PHAL::AlbanyTraits::Tangent>(workset);
    }
  }
...
*out << std::endl;
  ...
  if (trans) {
    overlapped_fpV = rcp(new Epetra_MultiVector(*(distParamLib->get(dist_param_name)->overlap_map()),
        V.NumVectors()));
  }
  else {
    overlapped_fpV = rcp(new Epetra_MultiVector(*(disc->getOverlapMap()), fpV.NumVectors()));
  }
  ...
  if (trans) {
    Teuchos::RCP<Epetra_Import>& importer = solMgr->get_importer();
    overlapped_V = rcp(new Epetra_MultiVector(
              *(disc->getOverlapMap()),
              V.NumVectors()));
    overlapped_V->Import(*V_bc, *importer, Insert);
  }
  else {
    overlapped_V = rcp(new Epetra_MultiVector(
...
//      loadBasicWorksetInfo( workset, overlapped_x, overlapped_xdot,
    ...
    for (int ws=0; ws < numWorksets; ws++) {
      ...
      if (nfm!=Teuchos::null)
        nfm->evaluateFields<PHAL::AlbanyTraits::DistParamDeriv>(workset);
    }
  }
  ...
  if (trans) {
    distParamLib->get(dist_param_name)->export_add(fpV, *overlapped_fpV);
  }
  else {
...
*out << std::endl;
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws=0; ws < numWorksets; ws++) {
      ...
      if (nfm!=Teuchos::null)
        nfm->evaluateFields<PHAL::AlbanyTraits::SGResidual>(workset);
    }
  }
...
*out << std::endl;
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws=0; ws < numWorksets; ws++) {
      ...
      if (nfm!=Teuchos::null)
        nfm->evaluateFields<PHAL::AlbanyTraits::SGJacobian>(workset);
    }
  }
  ...
  Stokhos::EpetraMultiVectorOrthogPoly* sg_fVp)
{
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws=0; ws < numWorksets; ws++) {
      ...
      if (nfm!=Teuchos::null)
        nfm->evaluateFields<PHAL::AlbanyTraits::SGTangent>(workset);
    }
  }
...
*out << std::endl;
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws=0; ws < numWorksets; ws++) {
      ...
      if (nfm!=Teuchos::null)
        nfm->evaluateFields<PHAL::AlbanyTraits::MPResidual>(workset);
    }
  }
...
*out << std::endl;
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws=0; ws < numWorksets; ws++) {
      ...
      if (nfm!=Teuchos::null)
        nfm->evaluateFields<PHAL::AlbanyTraits::MPJacobian>(workset);
    }
  }
  ...
  Stokhos::ProductEpetraMultiVector* mp_fVp)
{
  ...
  // Set data in Workset struct, and perform fill via field manager
  {
    ...
    for (int ws=0; ws < numWorksets; ws++) {
      ...
      if (nfm!=Teuchos::null)
        nfm->evaluateFields<PHAL::AlbanyTraits::MPTangent>(workset);
    }
  }
...
void Albany::Application::postRegSetup(std::string eval)
{
  ...
  if (eval=="Residual") {
    ...
    if (nfm!=Teuchos::null)
      nfm->postRegistrationSetupForType<PHAL::AlbanyTraits::Residual>(eval);
  }
    ...
    if (nfm!=Teuchos::null)
      nfm->postRegistrationSetupForType<PHAL::AlbanyTraits::Jacobian>(eval);
  }
    ...
    if (nfm!=Teuchos::null)
      nfm->postRegistrationSetupForType<PHAL::AlbanyTraits::Tangent>(eval);
  }
    ...
    if (nfm!=Teuchos::null)
      nfm->postRegistrationSetupForType<PHAL::AlbanyTraits::SGResidual>(eval);
  }
    ...
    if (nfm!=Teuchos::null)
      nfm->postRegistrationSetupForType<PHAL::AlbanyTraits::SGJacobian>(eval);
  }
    ...
    if (nfm!=Teuchos::null)
      nfm->postRegistrationSetupForType<PHAL::AlbanyTraits::SGTangent>(eval);
  }
    ...
    if (nfm!=Teuchos::null)
      nfm->postRegistrationSetupForType<PHAL::AlbanyTraits::MPResidual>(eval);
  }
    ...
    if (nfm!=Teuchos::null)
      nfm->postRegistrationSetupForType<PHAL::AlbanyTraits::MPJacobian>(eval);
  }
    ...
    if (nfm!=Teuchos::null)
      nfm->postRegistrationSetupForType<PHAL::AlbanyTraits::MPTangent>(eval);
  }
#endif //ALBANY_SG_MP
  else

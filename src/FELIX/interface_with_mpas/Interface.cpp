	                              ...
	                              double * /*heatIntegral_F*/, double * /*viscosity_F*/)
    {
		...
		if(!isDomainEmpty)
		{
			...
			for ( UInt j = 0 ; j < numVertices3D ; ++j )
		    {
			   ...
			   double* sHeight = stk_classic::mesh::field_data(*meshStruct->metaData->get_field<stk_classic::mesh::Field<double> >("surface_height"), node);
			   sHeight[0] = elevationData[ib];
			   double* thickness = stk_classic::mesh::field_data(*meshStruct->metaData->get_field<stk_classic::mesh::Field<double> >("thickness"),node);
			   ...
			   if(il ==0)
			   {
				   double* beta = stk_classic::mesh::field_data(*meshStruct->metaData->get_field<stk_classic::mesh::Field<double> >("basal_friction"), node);
			...
			for ( UInt j = 0 ; j < numPrisms ; ++j )
			{
			   ...
			   for(int iTetra=0; iTetra<3; iTetra++)
			   {
				   ...
				   double* temperature = stk_classic::mesh::field_data(*meshStruct->metaData->get_field<stk_classic::mesh::Field<double> >("temperature"), elem);

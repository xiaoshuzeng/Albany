...
{  
  ...
  bool compute_cloud_physics = xzhydrostatic_params->get<bool>("Compute Cloud Physics", false); 
...
{ 
  ...
  if (compute_cloud_physics) {
    ...
    for (int cell=0; cell < numCells; ++cell) {
      for (int qp=0; qp < numQPs; ++qp) {
        for (int level=0; level < numLevels; ++level) { 
          TempSrc                       (cell,qp,level) += 0;
          TracerSrc[namesToSrc["Vapor"]](cell,qp,level) += 0;
          TracerSrc[namesToSrc["Cloud"]](cell,qp,level) += 0;
          TracerSrc[namesToSrc["Rain"]] (cell,qp,level) += 0;
        }
      }
    }
